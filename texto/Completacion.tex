...
\[
  \begin{array}{lcl}
    ...
    $\texttt{Simplificar identidad}$   & \dfrac{E \cup \{ s\approx' t\},R}{E \cup \{ u \approx t\},R} & $Si $ s \rightarrow_R\\ \\
...
\item \texttt{(minRegla rl n r1 r2)} \index{minRegla} divide un
...
\item \texttt{(escogeRegla r)} \index{escogeRegla} calcula la regla
...
\item \texttt{(anadeRegla (l,r) e s r)} \index{anadeRegla} es el paso
...
\item \texttt{(orienta ord trip)} \index{orienta} es el segundo bucle
...
\item \texttt{(completa ord ee)} \index{completa} es el resultado de aplicar a ee, el algoritmo de completación de Huet. Por ejemplo,
